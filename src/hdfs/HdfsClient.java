...
public class HdfsClient {
    ...
    private static int numPorts[] = {3100, 3200, 3000, 3300, 3400};
    private static String nomMachines[] = {"vador", "leia", "yoda", "luke", "dragon"};
    final static int nbServers = 2;
    ...
    public static void HdfsWrite(Type fmt, String localFSSourceFname, int repFactor) {
         try {
            ...
            for (int i=0; i < nbfragments ; i++){
                ...
                Socket socket = new Socket (nomMachines[t], numPorts[t]);
                ObjectOutputStream objectOS = new ObjectOutputStream(socket.getOutputStream());
                objectOS.writeObject("CMD_WRITE" + "/@/" + localFSSourceFname + "_" + Integer.toString(i) + "/@/" + fragment);
                objectOS.close();
                socket.close();
            }
            fichier.close();
    ...
    public static void HdfsRead(String hdfsFname, String localFSDestFname) {
        ...
        try {
            ...
            for (int i = 0; i < nbfragments; i++) {
                ...
                objectOS.writeObject("CMD_WRITE" + "/@/" + hdfsFname +"_"+ Integer.toString(i)+ "-res");
